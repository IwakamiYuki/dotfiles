...
- **事実確認** — 情報源を自ら確認し、憶測を事実として述べない
  - **Web 調査を自動実施**: 時間依存情報（バージョン、API、セキュリティ、ベストプラクティス）、設計・実装方針の判断が必要な場合、確信がない情報
...
### Markdown ファイル: ユーザーレビューが必要な場合

以下のような重要なドキュメントを生成した場合、**Claude はユーザーに内容を確認してもらう**ために review-markdown スキルを活用する：

**対象ドキュメント:**
- 技術仕様書（API 仕様、システム設計、アーキテクチャ設計書）
- プロダクト・提案書（プロジェクト構想、改善提案、企画書）
- ユーザー向けドキュメント（README、Getting Started、チュートリアル）
- 設定・運用ドキュメント（セットアップ手順、トラブルシューティング、管理ガイド）

**活用方法:**

ドキュメント生成後、review-markdown スキルを自動的に実行：

```
【ユーザーレビュー】
このドキュメントをレビューしてほしい。
review-markdown スキルで Electron UI を開いて、コメントを追加してください。
```

**スキル起動時のフロー:**
- Electron アプリで Markdown を表示
- ユーザーがコメント追加（行単位、重大度指定）
- 完了後、コメント統計と改善提案を自動生成

### その他の Markdown ファイル

表示確認程度で十分な場合（簡単な説明、リンク集、一時的なメモなど）：
