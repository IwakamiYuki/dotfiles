...
**必須**: 専用ツールを使用（変更履歴追跡とエラーハンドリングのため）
1. 既存ファイル編集 → `Edit`
2. 新規ファイル作成 → `Write`
3. ファイル読み込み → `Read`

**禁止**: `cat > file`、`echo > file`、heredoc、`sed`、`awk` でのファイル操作
**例外**: 複数ファイル操作を一括で行うシェルスクリプト内のみ許可
...
**備考**: このファイルは全プロジェクト共通設定。プロジェクト固有指示は各プロジェクトルートに `CLAUDE.md` を作成。
