...
### Markdown ファイルのレビューが必要な場合

技術仕様書、API ドキュメント、設計書、README など、品質確認が必要な Markdown ファイルを生成した場合：

- **推奨**: `review-markdown` スキルを使用して MD Review MCP でレビュー
- **トリガー**: 「このドキュメントをレビューしてほしい」「品質確認してほしい」など、ユーザが明示的に依頼した場合、スキルが自動で起動
- **利点**: 段階的なコメント収集、重大度別分類、改善提案が得られる

### その他の Markdown ファイル

表示確認程度で十分な場合（簡単な説明、リンク集など）：
